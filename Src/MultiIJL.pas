...
////////////////////////////////////////////////////////////////////////////
  ...
  TJPEG_PROPERTIES = record
    ...
    jpeg_comment      : PAnsiChar;     // default = NULL
...
const
  OriginalDllName : AnsiString = 'Ijl15.dll';
...
type
  ...
  private
    ...
    ijlGetLibVersion : function : PIJLibVersion; stdcall;
    ijlErrorStr : function(Code : Integer) : PAnsiChar; stdcall;
  public
    constructor Create(ADllName:AnsiString = 'Ijl15.dll'); reintroduce;
...
constructor TMultiIJL.Create(ADllName: AnsiString);
begin
  inherited Create;

  FisLoaded := false;

  DLLHandle := LoadLibraryA(PAnsiChar(ADllName));
  if DLLHandle < 32 then raise Exception.Create('Dll LoadError');

  @ijlInit := GetProcAddress(DllHandle, PAnsiChar('ijlInit'));
  if @ijlInit = nil then raise Exception.Create('ijlInit LoadError');

  @ijlFree := GetProcAddress(DllHandle, PAnsiChar('ijlFree'));
  if @IjlFree = nil then raise Exception.Create('ijlFree LoadError');

  @ijlRead := GetProcAddress(DllHandle, PAnsiChar('ijlRead'));
  if @ijlRead = nil then raise Exception.Create('ijlRead LoadError');

  @ijlWrite := GetProcAddress(DllHandle, PAnsiChar('ijlWrite'));
  if @ijlWrite = nil then raise Exception.Create('ijlWrite LoadError');

  @ijlGetLibVersion := GetProcAddress(DllHandle, PAnsiChar('ijlGetLibVersion'));
  if @ijlGetLibVersion = nil then raise Exception.Create('ijlGetLibVersion LoadError');

  @ijlErrorStr := GetProcAddress(DllHandle, PAnsiChar('ijlErrorStr'));
  if @ijlErrorStr = nil then raise Exception.Create('ijlErrorStr LoadError');

  FisLoaded := true;
end;

destructor TMultiIJL.Destroy;
begin
...
procedure TMultiIJL.BitmapToJpeg(ASrc:pointer; AWidth,AHeight:integer;
...
procedure TMultiIJL.BitmapToJpegCopy(AJpegBuffer:Pointer; ASrc:pointer;
  AWidth,AHeight:integer; ADst:pointer; var ADstSize:integer;
  AQuality:integer=100);
var
  ErrorCode : Integer;
  IJLData: TJPEG_CORE_PROPERTIES;
begin
...
procedure TMultiIJL.JpegToBitmap(ASrc: pointer; ASrcSize: integer;
  var ADst: pointer; var ADstSize, AWidth, AHeight: integer);
var
  IJLData: TJPEG_CORE_PROPERTIES;
  ErrorCode : Integer;
begin
...
procedure TMultiIJL.JpegToBitmapCopy(ASrc:pointer; ASrcSize:integer;
  ADst:pointer; var ADstSize,AWidth,AHeight:integer);
var
  IJLData: TJPEG_CORE_PROPERTIES;
  ErrorCode : Integer;
begin
